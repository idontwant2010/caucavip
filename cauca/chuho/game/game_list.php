...
$prev30_from = (clone $today)->modify('-30 day');
$prev30_to = (clone $today)->modify('-8 day');
$prev7_from  = (clone $today)->modify('-7 day');
$prev7_to  = (clone $today)->modify('-2 day');
$yesterday   = (clone $today)->modify('-1 day');
$tomorrow    = (clone $today)->modify('+1 day');
$next7_from  = (clone $today)->modify('+2 day');
$next7_to  = (clone $today)->modify('+7 day');
$next30_from = (clone $today)->modify('+8 day');
$next30_to = (clone $today)->modify('+30 day');
...
";
if ($useDate) {
  $sql .= " AND g.ngay_to_chuc BETWEEN :fromAll AND :toAll ";
}
...
<body class="bg-light">
  <div class="container py-4">
    ...
    <div class="row g-3" id="gridGames">
      ...
      $renderBucket = function ($bucketKey, $items) use ($statusMap, $tz) {
        foreach ($items as $g) {
          ob_start();
          render_card($g, $statusMap, $tz);
          $html = ob_get_clean();
          $html = preg_replace(
            '#class="col-12 col-md-6 col-xl-4 game-card"#',
  ...
  <script>
    ...
    const counts = {
      prev30: <?= (int)$count['prev30'] ?>,
      prev7: <?= (int)$count['prev7'] ?>,
      yesterday: <?= (int)$count['yesterday'] ?>,
      today: <?= (int)$count['today'] ?>,
      tomorrow: <?= (int)$count['tomorrow'] ?>,
      next7: <?= (int)$count['next7'] ?>,
    ...
    function applyFilter(rangeKey) {
      ...
      if (none && !document.getElementById('emptyMsg')) {
        ...
        div.textContent = 'Không có game trong mốc thời gian này.';
