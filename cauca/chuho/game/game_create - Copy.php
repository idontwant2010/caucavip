...
if (!$ho) {
  ...
  echo "Hồ không hợp lệ hoặc không đủ điều kiện tạo game.";
  exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (!$dNgay) $errors[] = "Ngày tổ chức không hợp lệ (YYYY-MM-DD).";
    ...
    $dNgay->setTime(0, 0, 0);
    if ($dNgay < $today) $errors[] = "Ngày tổ chức phải từ hôm nay trở đi.";
  }
      ...
      $ins = $pdo->prepare("
        ...
        VALUES
          (:ho_cau_id, :chuho_id, :creator_id, :hinh_thuc_id,
           ...
           :dong_dk, :tien_cuoc, :phi_game, :phi_ho, NULL, 'dang_mo_dang_ky')
      ");
      $ins->execute([
        'ho_cau_id'        => $ho_id,
        'chuho_id'         => $chuho_id,
