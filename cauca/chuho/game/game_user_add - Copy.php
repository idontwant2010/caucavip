...
if ($game_id <= 0) {
  ...
  echo "Thiếu tham số game_id.";
  exit;
}

// 1) Xác thực game thuộc chủ hồ này và đang tồn tại
$sqlGame = "
  SELECT g.*, h.ten_ho, h.so_cho_ngoi
  FROM game_list g
...
if (!$game) {
  ...
  echo "Game không hợp lệ hoặc không thuộc quyền quản lý.";
  exit;
}
            ...
            <div class="table-responsive">
              <table class="table table-sm align-middle mb-0">
                ...
                <tbody>
                  <?php if (!$rows): ?>
                    <tr>
                      <td colspan="5" class="text-center text-muted py-3">Chưa có người chơi.</td>
                    </tr>
                    <?php else: $i = 1;
                    foreach ($rows as $r): ?>
                      <tr>
