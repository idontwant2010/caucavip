...
// Kiểm tra đã chia hiệp 1 chưa
$stmt = $pdo->prepare("SELECT COUNT(*) FROM giai_schedule WHERE giai_id = ? AND so_hiep = 1 AND so_bang IN ('A','B','C','D','E','F','G','H','I','K','L','M')");
$stmt->execute([$giai_id]);
$da_chia_hiep_1 = $stmt->fetchColumn();
if (!$da_chia_hiep_1) {
    echo "<script>alert('Chưa chia bảng hiệp 1'); window.location.href='my_giai_detail_step_2.php?id={$giai_id}';</script>";
    exit;
}
...
// Lấy dữ liệu hiệp 1
$stmt = $pdo->prepare("SELECT user_id, so_bang, is_bien FROM giai_schedule WHERE giai_id = ? AND so_hiep = 1 ORDER BY so_bang, vi_tri_ngoi");
$stmt->execute([$giai_id]);
$ds_hiep1 = $stmt->fetchAll();

// Gom về hiệp 2 theo bảng mới
$bang_2 = []; // [B => [], C => [], ...]
foreach ($ds_hiep1 as $row) {
...
foreach ($bang_2 as $bang => $users) {
    ...
    // Gán người từng ngồi biên → vị trí giữa
    ...
    // Gán phần còn lại (có thể biên hoặc giữa)
    ...
    foreach ($vi_tri_arr as $row) {
        $stmt2 = $pdo->prepare("UPDATE giai_schedule SET so_bang = ?, vi_tri_ngoi = ?, is_bien = ? WHERE giai_id = ? AND user_id = ? AND so_hiep = 2");
        $stmt2->execute([$row['so_bang'], $row['vi_tri_ngoi'], $row['is_bien'], $giai_id, $row['user_id']]);
    }
}
// ✅ Cập nhật trạng thái giải sang 'dang_dau_hiep_2'
$stmt = $pdo->prepare("UPDATE giai_list SET status = 'dang_dau_hiep_2' WHERE id = ?");
$stmt->execute([$giai_id]);
...
echo "<script>alert('✅ Đã chia bảng và vị trí hiệp 2 thành công!'); window.location.href='my_giai_detail_step_2.php?id={$giai_id}&hiep={$hiep_hien_tai}';</script>";
