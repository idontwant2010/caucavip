...
<div class="container mt-4">
  <h4 class="mb-4">🏆 Danh sách hồ cho phép tổ chức giải</h4>
  <form method="get" class="row g-2 mb-4">
    <div class="col-md-4">
      <select name="tinh_id" class="form-select">
        <option value="0">-- Tỉnh/Thành --</option>
        <?php foreach ($ds_tinh as $tinh): ?>
          <option value="<?= $tinh['id'] ?>" <?= ($tinh['id'] == $tinh_id) ? 'selected' : '' ?>><?= htmlspecialchars($tinh['ten_tinh']) ?></option>
        <?php endforeach; ?>
      </select>
    </div>
    <div class="col-md-4">
      <select name="loai_ca_id" class="form-select">
        ...
        <?php foreach ($ds_loai_ca as $ca): ?>
          <option value="<?= $ca['id'] ?>" <?= ($ca['id'] == $loai_ca_id) ? 'selected' : '' ?>><?= htmlspecialchars($ca['ten_ca']) ?></option>
        <?php endforeach; ?>
      </select>
    </div>
    <div class="col-md-4">
      <input type="text" name="keyword" value="<?= htmlspecialchars($keyword) ?>" class="form-control" placeholder="Tìm kiếm từ khoá...">
    </div>
    <div class="col-12">
