...
// Lấy số dư từ bảng users
$stmt = $pdo->prepare("SELECT balance FROM users WHERE id = :id");
$stmt->execute(['id' => $user_id]);
$user = $stmt->fetch(PDO::FETCH_ASSOC);

// Nếu không có kết quả -> gán mặc định
$balance = isset($user['balance']) ? (float)$user['balance'] : 0;

// Lấy log biến động tài khoản
$stmt = $pdo->prepare("SELECT * FROM user_balance_logs WHERE user_id = :user_id ORDER BY id DESC");
                        ...
                        <?php foreach ($logs as $log): ?>
                            <tr>
                                ...
                                <td>
                                    <?php
                                        switch ($log['type']) {
											case 'nap': echo 'Nạp tiền balance'; break;
											case 'rut': echo 'Rút tiền balance'; break;

										    case 'booking_hold': echo 'Tạm giữ phí đặt booking'; break;
										    case 'booking_pay': echo 'Thanh toán booking'; break;
										    case 'booking_refund': echo 'Hoàn phí đặt booking'; break;
										    case 'booking_received': echo 'Nhận thanh toán từ chủ hồ'; break;
