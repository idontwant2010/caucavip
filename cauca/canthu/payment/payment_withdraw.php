...
function withdraw($pdo, $userId, $amount, $bankInfo, $feeValue) {
    ...
    // 2. Tính phí cố định
    $feeAmount = calcWithdrawFee($amount, $feeValue);
    $totalDeduct = $amount + $feeAmount;

    if ($totalDeduct > $balance) {
        throw new Exception("Số dư không đủ (bao gồm phí " . number_format($feeAmount) . " đ).");
    }

    // 3. Trừ tiền
    $newBalance = $balance - $totalDeduct;
    $pdo->prepare("UPDATE users SET balance = ? WHERE id = ?")
        ->execute([$newBalance, $userId]);

    // 4. Sinh mã ref_no
    $refNo = "WD" . date("YmdHis") . rand(100,999);

    // 5. Ghi log vào user_balance_logs
    $note = "Rút " . number_format($amount) . "đ về " . $bankInfo 
          . " (phí cố định: " . number_format($feeAmount) . "đ)";
...
// biến phí
function calcWithdrawFeeFixed($feeValue) {
    return (int)($feeValue ?? 0);
}
// random ref
function makeRefNo($prefix = 'WD') {
...
$balanceCurrent = (float)($user['balance'] ?? 0);
$bankAccount    = (string)($user['bank_account'] ?? '');
$bankInfo       = (string)($user['bank_info'] ?? '');

$success = null;
$error   = null;

// Lưu ý: $feeValue bạn đã có ở đâu đó (config/admin). Nếu chưa có, gán tạm ở đây
$feeValue = isset($_POST['fee_value']) ? (int)$_POST['fee_value'] : 0;

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        // Lấy input
        $amount    = (int)($_POST['amount'] ?? 0);
        $bankAccIn = trim((string)($_POST['bank_account'] ?? $bankAccount));
        $bankInfIn = trim((string)($_POST['bank_info'] ?? $bankInfo));

        if ($amount <= 0) {
            throw new Exception('Số tiền rút không hợp lệ.');
        }
        if ($bankAccIn === '' || $bankInfIn === '') {
            throw new Exception('Thiếu thông tin ngân hàng nhận tiền.');
        }

        // Phí cố định
        $feeAmount   = calcWithdrawFeeFixed($feeValue);
        $totalDeduct = $amount + $feeAmount;

        // Bắt đầu transaction + khoá hàng user để tránh race condition
        $pdo->beginTransaction();

        // Khóa user row
        $stLock = $pdo->prepare("SELECT balance FROM users WHERE id = ? FOR UPDATE");
        $stLock->execute([$uid]);
        $balanceBefore = (float)$stLock->fetchColumn();

        if ($totalDeduct > $balanceBefore) {
            throw new Exception("Số dư không đủ (bao gồm phí " . number_format($feeAmount) . " đ).");
        }

        // Trừ tiền
        $balanceAfter = $balanceBefore - $totalDeduct;
        $stUpd = $pdo->prepare("UPDATE users SET balance = ? WHERE id = ?");
        $stUpd->execute([$balanceAfter, $uid]);

        // Tạo ref_no + note
        $refNo = makeRefNo('WD');
        $note  = "Rút " . number_format($amount) . "đ về " . $bankInfIn
               . " (STK: " . $bankAccIn . ", phí cố định: " . number_format($feeAmount) . "đ)";

        // Ghi log vào user_balance_logs
        $stIns = $pdo->prepare("
            INSERT INTO user_balance_logs
              (user_id, change_amount, type, amount, note, ref_no, balance_before, balance_after)
            VALUES
        ...
        $stIns->execute([
            $uid,
            -$totalDeduct,  // change_amount luôn âm khi rút
            -$amount,        // amount: số tiền rút gốc (chưa gồm phí)
            $note,
            $refNo,
            $balanceBefore,
            $balanceAfter
        ...
        $balanceCurrent = $balanceAfter;
        $bankAccount    = $bankAccIn;
        $bankInfo       = $bankInfIn;

        $success = "Rút tiền thành công. Mã tham chiếu: {$refNo}. Số dư còn lại: " . number_format($balanceAfter) . " đ.";
    } catch (Exception $ex) {
